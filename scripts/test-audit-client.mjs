...
const p3BrowserSource = await readFile(new URL('./qa-p3-browser.mjs', import.meta.url), 'utf8');
assert.equal(p3BrowserSource.includes("Target.createTarget"), true, 'P3 browser gate musí umět vytvořit page target, pokud Chromium žádný automaticky nezaloží.');
assert.equal(p3BrowserSource.includes("const page=await pageTarget(port)"), true, 'P3 browser gate se nesmí připojovat přes slepý pages.find(...).webSocketDebuggerUrl před vytvořením targetu.');
